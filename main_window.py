...
class MainWindow(QOpenGLWindow):
    ...
    def __init__(self):
        super().__init__()
        self.resize(350, 350)
        self.setTitle("OpenGL 2.1, PyQt6, Python")
    ...
    def initializeGL(self):
        ...
        root = doc.object()
        # Width and height
        tw = root["meta"]["size"]["w"].toDouble()
        th = root["meta"]["size"]["h"].toDouble()
        # Frames
        frames = root["frames"]
        # Button normal
        buttonNormal = frames["button-normal.png"]
        frame1 = buttonNormal["frame"]
        f1x = frame1["x"].toDouble()
        f1y = frame1["y"].toDouble()
        f1w = frame1["w"].toDouble()
        f1h = frame1["h"].toDouble()
        # Button active
        buttonActive = frames["button-active.png"]
        frame2 = buttonActive["frame"]
        f2x = frame2["x"].toDouble()
        f2y = frame2["y"].toDouble()
        f2w = frame2["w"].toDouble()
        f2h = frame2["h"].toDouble()
